...
test('district 4 shows four ballot sections and only its five city-council candidates', async ({ page }) => {
  ...
  await expect(page.locator('[data-district="2026-lm-city-4"] [data-ballot-section]:visible')).toHaveCount(4);
  ...
  expect(await page.locator('[data-district="2026-lm-city-4"] [data-election="city-council"] [data-candidate-card] h3').allTextContents())
    .toEqual(['Peter BONKO, Bc.', 'Šimon CUPRA, Ing.', 'Jaroslav GREŠO', 'Marian MATEJKA', 'Jozef REPASKÝ, Bc.']);

  const blchac = page.locator('[data-district="2026-lm-city-4"] [data-election="mayor"] [data-candidate-id="candidate-1"]');
  await expect(blchac.locator('.candidate-card__history')).toContainText('Účasť vo voľbách od roku 2000: 14 ×');
  await expect(blchac.locator('.candidate-card__history')).toContainText('zvolenie za poslanca 9 ×');
  await expect(blchac.locator('.candidate-card__history')).toContainText('starosta/primátor 4 ×');
  await expect(blchac.locator('.candidate-card__history [data-source-attribution] a').first()).toHaveAttribute('href', /^https:/);
});
