...
test('comparison keeps valid candidate URL order and renders eight common categories', async ({ page }) => {
  await page.goto(`${BASE}porovnat/?kandidat=${SECOND}&kandidat=${FIRST}`);
...
test('comparison rejects invalid, duplicate, and oversized URL selections', async ({ page }) => {
  await page.goto(`${BASE}porovnat/?kandidat=${FIRST}&kandidat=${FIRST}`);
  await expect(page.getByRole('status')).toContainText('neplatný');
  await expect(page.locator('[data-comparison-row]')).toHaveCount(0);

  await page.goto(`${BASE}porovnat/?kandidat=${FIRST}&kandidat=neexistuje`);
  await expect(page.getByRole('status')).toContainText('neplatný');

  await page.goto(`${BASE}porovnat/?kandidat=candidate-1&kandidat=candidate-2&kandidat=candidate-3&kandidat=candidate-4&kandidat=candidate-5`);
...
test('comparison keeps state only in URL and browser storage remains empty', async ({ page, context }) => {
  await page.goto(`${BASE}porovnat/?kandidat=${FIRST}&kandidat=${SECOND}`);
...
test('voting guide states four ballots and official selection limits', async ({ page }) => {
  ...
  await expect(page.getByText('1 hlas pre predsedu samosprávneho kraja (Žilinský samosprávny kraj)')).toBeVisible();
  await expect(page.getByText('najviac 6 kandidátov')).toBeVisible();
  await expect(page.getByText(/Oficiálne materiály vedú voľbu primátora mesta Liptovský Mikuláš/)).toBeVisible();
